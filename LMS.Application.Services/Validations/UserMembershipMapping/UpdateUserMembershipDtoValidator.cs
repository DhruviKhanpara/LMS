...
public class UpdateUserMembershipDtoValidator : AbstractValidator<UpdateUserMembershipDto>
{
    public UpdateUserMembershipDtoValidator()
    {
        RuleFor(userMembership => userMembership.Id)
            .NotEmpty().WithMessage("membership Id is required")
            .GreaterThan(0).WithMessage("User Membership Id ,must be grester than 0");
